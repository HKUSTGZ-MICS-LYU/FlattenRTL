    ...
    def _traverse_children(self,ctx):
        ...
        if isinstance(ctx,TerminalNodeImpl):
            ...
            for child in ctx.getChildren():
                # Dont change the name of the parameter like N in adder_4nbit #(N) lower_half
                if isinstance(child,SystemVerilogParser.Param_expressionContext):
                    continue
                if isinstance(child,SystemVerilogParser.Simple_identifierContext):
                    if isinstance(
                        ...
                        SystemVerilogParser.Name_of_instanceContext,
                    ):
                        child.start.text = (
                            ""
...
def pyflattenverilog(design: str, top_module: str, exlude_module : set):
    ...
    flatten_design = replace_module(design, top_module, extract_module(visitor.tmp_design, top_module))
